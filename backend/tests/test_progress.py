...
@pytest.mark.asyncio
async def test_progress_stats_calculation(db_session, test_tenant_id, test_user_id):
    """Verify completion percentage statistics for tenant."""
    # 1. Setup sample contents for the tenant
    c1 = AcademyContent(id=uuid.uuid4(), tenant_id=test_tenant_id, type=ContentType.SHORT, locale="tr", title="S1", video_url="v1", status=ContentStatus.PUBLISHED, order=1)
    c2 = AcademyContent(id=uuid.uuid4(), tenant_id=test_tenant_id, type=ContentType.SHORT, locale="tr", title="S2", video_url="v2", status=ContentStatus.PUBLISHED, order=2)
    c3 = AcademyContent(id=uuid.uuid4(), tenant_id=test_tenant_id, type=ContentType.SHORT, locale="tr", title="S3", video_url="v3", status=ContentStatus.PUBLISHED, order=3)
    ...
    # 3. Get Stats
    stats = await service.get_stats(test_tenant_id, ContentType.SHORT)
